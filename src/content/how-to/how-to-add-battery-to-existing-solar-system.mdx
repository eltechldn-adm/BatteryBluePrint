...
**Lesson:** Before committing to an AC-coupled retrofit, always price the cost of replacing the existing inverter to DC-coupled hybrid architecture — particularly if the inverter is more than 7 years old. The break-even between AC retrofit and DC replacement is often closer than expected. Use the battery sizing calculator to model both scenarios, and check current system pricing in the [UK cost guide](/cost/solar-battery-cost-uk-2026).
