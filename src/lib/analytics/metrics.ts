...
export async function getAnalyticsSummary(): Promise<AnalyticsSummary> {
    ...
    const rawEvents = await kv.lrange('events_log', 0, sampleSize - 1);
...
export async function getDailySummary(): Promise<DailyReportSummary> {
    ...
    const eventLogs = await kv.lrange('events_log', 0, 4999);
    const leadLogs = await kv.lrange('leads_log', 0, 999);

    // Filter last 24h
    const recentEvents = eventLogs.filter((e: any) => {
        if (!e.timestamp) return false;
        return new Date(e.timestamp).getTime() > twentyFourHoursAgo;
    });
    const recentLeads = leadLogs.filter((l: any) => {
        if (!l.timestamp) return false;
        return new Date(l.timestamp).getTime() > twentyFourHoursAgo;
    });

    // Calc Submissions
    const calculatorSubmissions = recentEvents.filter((e: any) =>
        e.type === 'calculator_submit' || e.action === 'calculator_submit'
    ).length;

    // Blueprint Downloads
    const blueprintDownloads = recentEvents.filter((e: any) =>
